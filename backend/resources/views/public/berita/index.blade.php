...
@section('content')
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-14" data-testid="berita-page">
        <div class="max-w-3xl">
            <div class="text-xs font-bold uppercase tracking-[0.2em] text-brand-700"><span class="inline-block w-8 h-px bg-brand-500 mr-2 align-middle"></span>Berita</div>
            <h1 class="font-display text-5xl lg:text-6xl font-black text-brand-950 mt-4 tracking-tight leading-[0.95]">Cerita & kabar <span class="font-editorial italic text-brand-700">terbaru</span> madrasah.</h1>
        </div>
        <div class="mt-10 flex flex-col md:flex-row gap-3">
            ...
            <div class="flex gap-2 overflow-x-auto">
                ...
                @foreach ($chips as $c)
                    ...
                    @endphp
                    <a href="{{ $href }}" data-testid="berita-filter-{{ strtolower($c) }}" class="px-4 py-2 rounded-xl text-sm font-semibold whitespace-nowrap transition {{ $active ? 'bg-brand-950 text-white' : 'bg-white border border-slate-200 text-brand-900 hover:bg-brand-50' }}">{{ $c }}</a>
                @endforeach
            </div>
        </div>
            ...
            @foreach ($news as $n)
                ...
                <a href="/berita/{{ $n['slug'] ?? ($n['id'] ?? '') }}" class="group" data-testid="berita-card-{{ $n['id'] ?? '' }}">
                    <div class="aspect-[4/3] rounded-3xl overflow-hidden mb-4 relative">
                        ...
                        <div class="absolute top-3 left-3 bg-white/95 backdrop-blur text-[10px] font-bold uppercase tracking-wider text-brand-800 rounded-full px-2.5 py-1">{{ $n['category'] ?? '' }}</div>
                    </div>
                    <h3 class="font-display font-bold text-xl text-brand-950 leading-tight group-hover:text-brand-700 transition">{{ $n['title'] ?? '' }}</h3>
