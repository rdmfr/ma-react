...
@section('content')
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-14" data-testid="kontak-page">
        <div class="max-w-3xl">
            <div class="text-xs font-bold uppercase tracking-[0.2em] text-brand-700"><span class="inline-block w-8 h-px bg-accent-500 mr-2 align-middle"></span>Hubungi Kami</div>
            <h1 class="font-display text-5xl lg:text-6xl font-black text-brand-950 mt-4 tracking-tight leading-[0.95]">Jangan ragu untuk <span class="font-editorial italic text-accent-600">menyapa</span>.</h1>
        </div>
        <div class="mt-12 grid lg:grid-cols-12 gap-8">
            <div class="lg:col-span-5 space-y-5">
                <div class="bg-brand-950 text-white rounded-3xl p-8 relative overflow-hidden">
                    ...
                    <div class="mt-6 space-y-5 relative">
                        <div class="flex gap-3"><i data-lucide="map-pin" class="w-5 h-5 text-accent-200 shrink-0 mt-0.5"></i><div><div class="text-xs uppercase tracking-wider text-accent-200">Alamat</div><div class="mt-1">{{ $branding['address'] ?? '' }}</div></div></div>
                        <div class="flex gap-3"><i data-lucide="mail" class="w-5 h-5 text-accent-200 shrink-0 mt-0.5"></i><div><div class="text-xs uppercase tracking-wider text-accent-200">Email</div><div class="mt-1">{{ $branding['email'] ?? '' }}</div></div></div>
                        <div class="flex gap-3"><i data-lucide="phone" class="w-5 h-5 text-accent-200 shrink-0 mt-0.5"></i><div><div class="text-xs uppercase tracking-wider text-accent-200">Telepon</div><div class="mt-1">{{ $branding['phone'] ?? '' }}</div></div></div>
                    </div>
                    <div class="mt-8 pt-5 border-t border-brand-800 flex gap-2 relative">
                        <a href="#" class="w-10 h-10 rounded-full glass-dark flex items-center justify-center hover:bg-accent-500/25 transition"><i data-lucide="instagram" class="w-4 h-4"></i></a>
                        <a href="#" class="w-10 h-10 rounded-full glass-dark flex items-center justify-center hover:bg-accent-500/25 transition"><i data-lucide="facebook" class="w-4 h-4"></i></a>
                        <a href="#" class="w-10 h-10 rounded-full glass-dark flex items-center justify-center hover:bg-accent-500/25 transition"><i data-lucide="youtube" class="w-4 h-4"></i></a>
                    </div>
                </div>
            ...
            <div class="lg:col-span-7">
                <form method="POST" action="{{ route('kontak.submit') }}" class="bg-white rounded-3xl border border-slate-100 p-8 space-y-4">
                    ...
                    <button type="submit" data-testid="kontak-submit" class="inline-flex items-center gap-2 rounded-xl gradient-accent gradient-accent-hover text-white px-6 py-3.5 font-bold shadow-lg shadow-black/10 transition card-lift">
