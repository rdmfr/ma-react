    ...
    <div class="max-w-4xl mx-auto">
        ...
        <div class="bg-white rounded-3xl border border-brand-100 shadow-xl shadow-brand-900/5 p-8">
            <form action="{{ route('public.check-scores.search') }}" method="POST" class="flex flex-col sm:flex-row gap-4">
                ...
                <button type="submit" class="inline-flex items-center justify-center gap-2 px-8 py-4 gradient-accent gradient-accent-hover text-white rounded-2xl font-bold transition shadow-lg shadow-black/10 card-lift">
                    Cek Hasil
                </button>
            </form>
                    ...
                    <div class="mt-8">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            @foreach($scores as $score)
                                <div class="p-5 rounded-2xl border border-slate-100 hover:border-accent-200 hover:bg-accent-50/40 transition bg-white flex items-center justify-between group">
                                    <div>
                                        <div class="text-xs font-bold text-slate-400 uppercase tracking-wider">{{ $score['semester'] ?? 'Semester' }} {{ $score['academic_year'] ?? '' }}</div>
                                        <div class="font-bold text-brand-950 mt-0.5 group-hover:text-accent-700 transition">{{ $score['subject'] }}</div>
                                    </div>
                                    <div class="text-2xl font-black text-accent-700 bg-accent-50 w-14 h-14 rounded-xl flex items-center justify-center border border-accent-100 group-hover:bg-accent-600 group-hover:text-white transition">
                                        {{ $score['score'] }}
                                    </div>
                                </div>
                            @endforeach
        ...
        <div class="mt-12 text-center text-sm text-slate-500">
            <p>Butuh bantuan? Hubungi <a href="/kontak" class="text-accent-700 hover:text-accent-800 font-semibold underline">Layanan Bantuan Madrasah</a></p>
        </div>
    </div>
</div>
@endsection
