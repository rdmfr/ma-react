...
import { ArrowLeft, Mail, GraduationCap, BookOpen } from "lucide-react";
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-14" data-testid="guru-detail-page">
            ...
            <div className="grid lg:grid-cols-12 gap-10">
                <div className="lg:col-span-5">
                    <div className="aspect-[3/4] rounded-3xl overflow-hidden bg-brand-100"><img src={t.photo} alt={t.name} className="w-full h-full object-cover" /></div>
                </div>
                <div className="lg:col-span-7">
                    <div className="text-xs font-bold uppercase tracking-[0.2em] text-brand-700">{t.subject}</div>
                    <h1 className="font-display text-5xl font-black text-brand-950 mt-3 tracking-tight leading-[0.95]">{t.name}</h1>
                    <p className="mt-6 text-brand-800/85 text-lg leading-relaxed">{t.bio}</p>
                    <div className="mt-8 grid sm:grid-cols-2 gap-4">
                        <div className="bg-white rounded-2xl p-5 border border-slate-100">
                            <div className="flex items-center gap-2 text-brand-700"><GraduationCap className="w-4 h-4" /><span className="text-xs font-bold uppercase tracking-wider">Pendidikan</span></div>
                            <div className="mt-2 font-semibold text-brand-950">{t.education}</div>
                        </div>
                        <div className="bg-white rounded-2xl p-5 border border-slate-100">
                            <div className="flex items-center gap-2 text-brand-700"><BookOpen className="w-4 h-4" /><span className="text-xs font-bold uppercase tracking-wider">Mata Pelajaran</span></div>
