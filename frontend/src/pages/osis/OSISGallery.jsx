    ...
    return (
        <div data-testid="osis-gallery">
            <PageHeader title="Galeri Kegiatan" description="Unggah album foto kegiatan untuk ditampilkan di galeri publik." breadcrumbs={["OSIS", "Galeri"]}
                actions={<button onClick={() => { setForm({ title: "", cover: "", date: new Date().toISOString().slice(0, 10), count: 0 }); setEditor(true); }} className="inline-flex items-center gap-2 rounded-xl gradient-brand text-white px-5 py-2.5 text-sm font-bold"><Plus className="w-4 h-4" />Album Baru</button>} />
            <div className="border-2 border-dashed border-brand-200 rounded-3xl p-10 text-center bg-brand-50/40 mb-8">
                ...
                <div className="fixed inset-0 bg-brand-950/60 backdrop-blur-sm z-50 flex items-center justify-center p-4 sm:p-6 animate-fade-up" onClick={() => setEditor(false)}>
                    <div className="bg-white rounded-3xl max-w-xl w-full overflow-hidden" onClick={e => e.stopPropagation()}>
                        ...
                        <div className="p-7 space-y-4">
                            ...
                            <div><label className="text-sm font-semibold text-brand-950">Cover (URL)</label><input value={form.cover} onChange={e => setForm({ ...form, cover: e.target.value })} className="mt-1.5 w-full rounded-xl border border-slate-200 px-4 py-3 text-sm outline-none focus:border-brand-500" /></div>
                        ...
                        <div className="px-7 py-4 border-t border-slate-100 flex gap-3">
                            ...
                            <button
                                onClick={async () => {
                                    ...
                                    try {
                                        const created = await galleriesApi.createItem({ title: form.title, cover: form.cover, date: form.date, count: Number(form.count) || 0, status: "pending", submittedBy: user?.email });
