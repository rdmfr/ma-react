    ...
    return (
        <div data-testid="osis-announcements">
            <PageHeader title="Draft Pengumuman" description="Buat draft pengumuman untuk ditinjau admin." breadcrumbs={["OSIS", "Pengumuman"]}
                actions={<button onClick={() => { setForm({ title: "", content: "" }); setEditor(true); }} className="inline-flex items-center gap-2 rounded-xl gradient-brand text-white px-5 py-2.5 text-sm font-bold"><Plus className="w-4 h-4" />Draft Baru</button>} />
            <div className="space-y-3">
                {myItems.map((a, i) => (
                    <div key={a.id} className="bg-white rounded-2xl border border-slate-100 p-5 flex items-center gap-4 card-lift">
                        ...
                        <button className="w-9 h-9 rounded-xl border border-slate-200 text-brand-700 flex items-center justify-center hover:bg-brand-50"><Pencil className="w-3.5 h-3.5" /></button>
                    </div>
                ))}
            </div>
